...
func resolveInstalledRemoteImport(source, declaredVersion, cityRoot string) (string, error) {
	...
	// No packs.lock entry resolved. A bundled builtin source can still resolve
	// to the binary's canonical pin without a lock, which keeps cities
	// composable before the first "gc import install" writes the lock.
	if dir, ok, bErr := resolveBundledSourceWithoutLock(source, declaredVersion); ok {
		if bErr != nil {
			return "", fmt.Errorf("resolving remote import %s without lock: %w", source, bErr)
		}
		return dir, nil
	}
