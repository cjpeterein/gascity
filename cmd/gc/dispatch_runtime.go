...
func (s workflowEventScope) relevant(evt events.Event) bool {
	...
	routed := strings.TrimSpace(bead.Metadata["gc.routed_to"])
	runTarget := strings.TrimSpace(bead.Metadata["gc.run_target"])
	workflowKind := strings.TrimSpace(bead.Metadata["gc.kind"])
