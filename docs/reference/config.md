...
| `default_rig_includes` | []string |  |  | DefaultRigIncludes lists pack directories derived from [defaults.rig.imports] in the city root pack. "gc rig add" copies them into newly-added rigs' imports, and at runtime LoadWithIncludes expands the same defaults against every rig that does not already declare a matching [rigs.imports.&lt;name&gt;] binding. Populated from the pack by LoadWithIncludes; may also be authored directly in city.toml. |
